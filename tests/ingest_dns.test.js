...
module.exports = () => {
    return new Promise((resolve, reject) => { 
        googleDNS("ingest.vrcdn.live")
          .then((googleResults) => {
            logger("debug", `Getting local DNS results...`);
            logger('debug', `Current DNS servers: ${dns.getServers()}`)
            dns.resolve4("ingest.vrcdn.live", (error, localResults) => {
              if (error) {
              ...
              if (localResults.equalIgnoreOrder(googleResults)) {
                logger(
                  "success",
                  `Ingest DNS test passed, local results match Google: ${localResults}`
                );
                resolve(true);
              } else {
                logger(
